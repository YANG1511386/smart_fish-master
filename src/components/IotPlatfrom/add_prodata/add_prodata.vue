 ...
 title="创建配置"
  ...
  <a-form  :form="form" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }" @submit="handleSubmit">
    <!--<a-form-item label="ID">
      ...
      <a-form-item label="jsonKey">
      <a-input
       placeholder="请输入jsonKey"  v-decorator="['jsonKey', { rules: [{ required: true, message: 'Please input your jsonKey!' }] }]"
      />
    </a-form-item>
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
         ...
         // console.log(typeof(a))
          ...
          this_1.mypath=window.g.new_pro_data
          
          axios({
             
             url: window.g.new_pro_data,
             method:'post',
                 //发送格式为json
              headers:{
